...
Each block is mandatory and has to be filled in. This is what makes its syntax tedious and error-prone. x

We can define a `Problem` by using the DSL-access function `problem {}`. 

This populates *all* the default problem components (`bop`, `desc`, `logic`, `settings`, `eop`) to their default values (which, apart from `logic` which is where the actual code has to be written, is not relevant to the final problem solution).
...
- `flagShowSolution` matches `PProblem`, and will result in the interpreter printing out the problem analysis result;
